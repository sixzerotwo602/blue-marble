...
import { CardEffectType, CardKeepPolicy, CardPhasePolicy } from './enums';
import { GoldenKeyCard } from './types';
import { SPECIAL_TILE_INDICES } from './board-data';
...
export const GOLDEN_KEY_CARDS: GoldenKeyCard[] = [
  ...
  },
  {
    ...
    keepPolicy: CardKeepPolicy.DISCARD_BOTTOM,
    phasePolicy: CardPhasePolicy.ANYTIME,
    destinationIndex: SPECIAL_TILE_INDICES.ISLAND,
  },
  {
    id: 'welfare-payout',
    name: '사회복지기금 배당',
    ...
    keepPolicy: CardKeepPolicy.DISCARD_BOTTOM,
    phasePolicy: CardPhasePolicy.ANYTIME,
    destinationIndex: SPECIAL_TILE_INDICES.WELFARE_PAYOUT,
  },
  {
    id: 'world-tour',
    name: '세계일주 초대권',
  ...
  },
  {
    ...
    keepPolicy: CardKeepPolicy.DISCARD_BOTTOM,
    phasePolicy: CardPhasePolicy.ANYTIME,
    destinationIndex: SPECIAL_TILE_INDICES.SPACE_TRAVEL,
  },
  {
    id: 'cruise-travel',
    name: '유람선 여행',
  ...
  },
  {
    ...
    keepPolicy: CardKeepPolicy.DISCARD_BOTTOM,
    phasePolicy: CardPhasePolicy.ANYTIME,
    destinationIndex: SPECIAL_TILE_INDICES.START,
    destinationName: '출발',
    value: 200_000, // 월급 지급
  },
...
export function getKeepableCards(): GoldenKeyCard[] {
  const result: GoldenKeyCard[] = [];
  for (const card of GOLDEN_KEY_CARDS) {
    if (card.keepPolicy === CardKeepPolicy.KEEP_UNTIL_USE) {
      result.push(card);
    }
  }
  return result;
...
export function getSecondHalfOnlyCards(): GoldenKeyCard[] {
  const result: GoldenKeyCard[] = [];
  for (const card of GOLDEN_KEY_CARDS) {
    if (card.phasePolicy === CardPhasePolicy.SECOND_HALF_ONLY) {
      result.push(card);
    }
  }
  return result;
