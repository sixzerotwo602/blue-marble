...
- Q: 게임 시작 최소 인원? → A: **3명 이상 4명이하** (호스트가 시작 결정).
...
### Session 2026-01-09 (규칙 파일 일관성 분석 반영)

- Q: 건물 건설 비용? → A: **빌라 50,000원, 건물 150,000원, 호텔 250,000원** (VILLA_COST, BUILDING_COST, HOTEL_COST). 건설 비용은 모든 도시에 동일하게 적용.
- Q: 건물 건설 시 순차 강제? → A: **순차 제약 없음**. 자금만 충분하다면 한 턴에 빌라→건물→호텔까지 한 번에 건설 가능. 단, 빌라 없이 건물/호텔 건설은 불가.
- Q: 건물 매각 시 다운그레이드 가능? → A: **다운그레이드 없음**. 호텔 매각 시 호텔이 소멸하며 빌라/건물로 되돌아가지 않음. 매각가는 건설 비용의 100% 환급.
- Q: 탈것(콩코드/퀸엘리자베스/콜롬비아) 통행료? → A: **콩코드 30만원, 퀸엘리자베스 25만원, 콜롬비아 40만원**. 건물 건설 불가하며 고정 통행료 적용.
- Q: 대출 시스템(은행 대출)? → A: **MVP Out-of-Scope**. 규칙 파일에 정의된 대출 한도 100만원, 3바퀴 상환 규칙은 추후 확장 시 적용.
- Q: 황금열쇠 카드 덱 관리 규칙? → A: **순환 구조**. 카드는 덱 상단에서 뽑고 사용 후 덱 하단에 반환. 보관 카드(우대권, 탈출권)는 사용 전까지 반환 안 함.
- Q: 무인도 탈출 시 탈출권 은행 매각? → A: **가능 (200,000원)**. 단, 무인도에 갇힌 상태에서는 매각 불가 (탈출용으로만 사용).
- Q: 우대권(통행료 면제권) 사용 시점? → A: **통행료 발생 직전**. 타인 소유 땅 도착 시 "우대권 사용" 옵션이 제시되며, 사용 시 해당 턴 통행료 면제.
- Q: 황금열쇠 유지비(정기소득세/건물수리비/방범비) 적용 기준? → A: **보유 건물당 개별 적용**. 빌라/건물/호텔 각각 다른 비용이 적용되며, 건물이 없으면 비용 없음.
- Q: 세계일주 초대권 카드 효과? → A: **현재 위치에서 보드 한 바퀴 이동**. 출발 통과 시 월급(20만원) 수령 + 사회복지기금 접수 통과 시 적립금 수령.
...
- 대출 시스템 (은행 대출 한도 100만원, 3바퀴 상환 규칙 - 추후 확장 가능)
- 전반전/후반전 개념 (증서 5~6장 남을 시 경매 후 후반전 전환 - 추후 확장 가능)
- 옵션 게임 모드 (추가 자금 지급 + 증서 선분배 후 바로 후반전 - 추후 확장 가능)
...
| `ISLAND_TURNS`           | 3턴         | 무인도 대기 후 자동 탈출 턴 수       |
| `DOUBLE_LIMIT`           | 3회         | 연속 더블 제한 (초과 시 턴 종료)     |
| `VILLA_COST`             | 50,000원    | 빌라 건설 비용                       |
| `BUILDING_COST`          | 150,000원   | 건물 건설 비용                       |
| `HOTEL_COST`             | 250,000원   | 호텔 건설 비용                       |
| `CONCORDE_TOLL`          | 300,000원   | 콩코드여객기 통행료                  |
| `QUEEN_ELIZABETH_TOLL`   | 250,000원   | 퀸엘리자베스호 통행료                |
| `COLUMBIA_TOLL`          | 400,000원   | 콜롬비아호 통행료                    |
| `ALL_DISCONNECT_TIMEOUT` | 30분        | 전원 연결 끊김 시 게임 유지 시간     |
...
- **FR-034**: 시스템은 건물 건설 시 빌라 50,000원, 건물 150,000원, 호텔 250,000원의 비용을 차감해야 한다
- **FR-035**: 시스템은 건물 단계를 순차적으로만 허용해야 한다 (빈 땅→빌라→건물→호텔). 단, 한 턴에 연속 건설은 허용
- **FR-036**: 시스템은 건물 매각 시 건설 비용의 100%를 환급하고, 다운그레이드 없이 해당 건물을 소멸시켜야 한다
- **FR-037**: 시스템은 더블 주사위 시 추가 턴을 부여해야 하며, 3회 연속 더블 시 즉시 턴을 종료해야 한다 (무인도 이동 없음)
- **FR-038**: 시스템은 황금열쇠 카드를 덱 상단에서 뽑고, 사용 후 덱 하단에 반환해야 한다
- **FR-039**: 시스템은 보관 가능 카드(우대권, 무인도 탈출권)를 플레이어 인벤토리에 저장하고, 사용 시까지 덱에 반환하지 않아야 한다
- **FR-040**: 시스템은 무인도 탈출권을 무인도 외 상황에서 은행에 200,000원에 매각할 수 있도록 해야 한다
- **FR-041**: 시스템은 우대권 보유 시 타인 소유 땅 도착 시 "우대권 사용" 옵션을 제공하고, 사용 시 해당 턴 통행료를 면제해야 한다
- **FR-042**: 시스템은 황금열쇠 유지비 카드(정기소득세/건물수리비/방범비) 적용 시 보유 건물 종류별로 개별 비용을 계산해야 한다
- **FR-043**: 시스템은 황금열쇠 이동 카드 실행 중 출발 칸을 통과하면 월급(20만원)을 지급해야 한다 (무인도 이동 제외)
- **FR-044**: 시스템은 황금열쇠로 이동한 위치가 황금열쇠 칸인 경우 연쇄적으로 새 카드를 뽑도록 해야 한다
- **FR-045**: 시스템은 무인도에서 3턴 대기 시 비용 없이 자동 탈출을 처리해야 한다
- **FR-046**: 시스템은 전원 연결 끊김 시 30분간 게임 상태를 유지하고, 미복귀 시 게임을 자동 종료해야 한다 (endReason: timeout)

### Key Entities

- **GameRoom**: 게임방 정보 (방 코드, 상태, 생성 시간, 호스트)
- **Player**: 플레이어 정보 (닉네임, 색상, 현재 위치, 현금, 소유 자산, 연결 상태)
- **PlayerInventory**: 보관 카드 인벤토리 (우대권, 무인도 탈출권 - 최대 2장까지 보유 가능)
- **BoardTile**: 보드판 칸 정보 (위치, 이름, 종류, 가격, 임대료 테이블, 소유자, 건물 수준)
- **GoldenKeyCard**: 황금열쇠 카드 (ID, 내용, 효과 종류, 금액/목적지, 보관 가능 여부)
- **GoldenKeyDeck**: 황금열쇠 덱 상태 (카드 순서, 현재 인덱스 - 순환 관리)
- **FundPool**: 사회복지기금 적립금 (누적 금액, 마지막 기부자)
- **GameState**: 게임 상태 (현재 턴, 턴 순서, 게임 진행 상태, 일시 정지 여부, 연속 더블 횟수)
